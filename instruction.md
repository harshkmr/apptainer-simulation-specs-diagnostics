Administer and diagnose Apptainer container runtime execution environments, system resource limits, process signals, memory profiles, and simulation solver traces. Build an offline system administration diagnostic tool (`apptainer_diag`) packaged via `setuptools` with CLI entrypoint `apptainer-diag`.
...
2. **System Log & Spec File Parsers**:
